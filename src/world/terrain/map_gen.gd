...
@export var map_length: int = 0 # Width of the map in cells
@export var map_height: int = 0 # Height of the map in cells
@export var cell_size: Vector2 = Vector2(32, 32) # Size of each map cell in pixels
...
func initialize_map():
	if map_length <= 0 or map_height <= 0:
		push_error("Invalid map dimensions: length=%d, height=%d" % [map_length, map_height])
		return
		
	map_data = MapData.new(Vector2i(map_length, map_height), randi())
...
func sync_renderer_settings():
	if map_renderer:
		map_renderer.show_grid_lines = display_config.show_grid_lines
		map_renderer.show_coordinate_numbers = display_config.show_coordinate_numbers
		map_renderer.show_terrain_letters = display_config.show_terrain_letters
		map_renderer.show_density_values = display_config.show_density_values
		map_renderer.show_movement_costs = display_config.show_movement_costs
		map_renderer.show_resources = display_config.show_resources
