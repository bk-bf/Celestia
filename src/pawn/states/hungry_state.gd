...
# Remove the state_name declaration - it's likely defined in PawnState
...
# Remove the _init() method entirely
...
func update(delta):
    # Add a basic update method similar to other states
    # Check if job was canceled
    if pawn.current_job == null and is_searching_for_food:
...
func find_food():
    ...
    if food_position and is_valid_food_position(food_position):
        # Create an eating job with eat_until_full flag
        var job = EatingJob.new(food_position, "food", 1, 2.0)
        job.eat_until_full = true # Add this property to your EatingJob class
...
func check_needs_state():
    ...
    if state_machine.current_state == "Hungry":
        ...
        var satisfied_threshold = needs_database.hunger_config.thresholds.satisfied
        
        # If hunger is above the satisfied threshold, go back to idle
        if pawn.needs["hunger"].current_value >= satisfied_threshold:
